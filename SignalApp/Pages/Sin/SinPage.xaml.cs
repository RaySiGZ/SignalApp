...
namespace SignalApp.Pages.Sin
{
   /// <summary>
   /// Логика взаимодействия для SinPage.xaml
   /// </summary>
   public partial class SinPage : Page
   {
      ...
      private void DrawGraf(double amplitude, double frequency, int maxCount, int? periodCount)
      {
         ...
         graf = MathFunc.GenerateGraf(SignalType.Sine, amplitude, frequency, maxCount, periodCount);
      ...
      public SinPage(string fullPath, bool saveFile, double amplitude, double frequency, int maxCount, int? periodCount)
      {
         InitializeComponent();
